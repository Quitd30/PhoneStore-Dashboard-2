...
function showQuickViewModal(productData, isLoading = false) {
    ...
    if (isLoading || !productData) {
        ...
        if (productDiv) productDiv.classList.add('hidden');
    } else {
        // Hide loading and show product data
        if (loadingDiv) loadingDiv.classList.add('hidden');
        if (productDiv) productDiv.classList.remove('hidden');
...
function populateQuickViewData(data) {
    try {
        ...
        // Update colors
        updateQuickViewColors(data.colors || [], data.images || []);
...
function updateQuickViewImages(images, productName) {
    ...
    if (images && images.length > 0) {
        // Set main image with error handling - use imageUrl property
        ...
        mainImageEl.src = 'https://via.placeholder.com/400x400/EEE/999?text=No+Image';
...
window.addToCart = function(productId, productName, price, quantity = 1) {
    ...
    showNotification('Đang thêm vào giỏ hàng...', 'info', 1000);
...
window.addToWishlist = function(productId, productName) {
    // Placeholder wishlist function - implement later
    showNotification(`Đã thêm ${productName} vào danh sách yêu thích`, 'success');
};
...
// Helper functions for product validation
...
function setupQuickViewActions(data) {
    ...
    if (addToWishlistBtn) {
        addToWishlistBtn.onclick = () => {
            console.log('Add to wishlist clicked:', data.id, data.name);
            addToWishlist(data.id, data.name);
        };
    }
